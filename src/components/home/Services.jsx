...
import loftKitchen01 from "../../data/fotos-projects/loft-manhattan/COCINA_1.33.jpg";
import loftKitchen02 from "../../data/fotos-projects/loft-manhattan/COCINA_2.33.jpg";
import loftKitchen03 from "../../data/fotos-projects/loft-manhattan/COCINA_3.33.jpg";

const resolveImageSrc = (asset) => (typeof asset === "string" ? asset : asset?.src || "");

export function Services({ compact = false, includePremium = false, includeConsult = true }) {
    ...
    const loftKitchenSlides = [loftKitchen01, loftKitchen02, loftKitchen03].map(resolveImageSrc);
    ...
    const premiumPackage = {
        ...
        name: "Cocina Premium (Modelo Loft Manhattan)",
        ...
        ctaHref: `https://wa.me/${whatsappNumber}?text=${encodeURIComponent(
            "Hola Alfredo, quiero información sobre la Cocina Premium (Modelo Loft Manhattan) desde USD 18.000."
        )}`,
        recommended: false,
    };
    ...
    const consultPackage = {
        ...
        cardClass: "bg-[#0070BB] p-4 md:p-5 ring-blue-300 text-white hover:shadow-lg dark:bg-[#0070BB] dark:ring-blue-300 dark:text-white",
        darkCard: true,
    ...
    const packages = [
        standardPackage,
        ...(includePremium ? [premiumPackage] : []),
        ...(includeConsult ? [consultPackage] : []),
    ];
            ...
            {!compact && (
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                    {items.map((s, i) => (
                        ...
                        >
                            <div className="mb-4 inline-flex items-center justify-center rounded-2xl bg-zinc-100 p-3 text-zinc-900 transition-colors group-hover:bg-zinc-900 group-hover:text-white dark:bg-zinc-800 dark:text-zinc-100 dark:group-hover:bg-zinc-100 dark:group-hover:text-zinc-900">
                                {s.icon}
                            </div>
            ...
            <div className={compact ? "mt-0" : "mt-24"}>
                ...
                <div className={`mt-16 grid items-start gap-8 ${packages.length > 2 ? "lg:grid-cols-2 xl:grid-cols-3" : "lg:grid-cols-2"}`}>
                    {packages.map((pkg) => {
                        ...
                        const isDarkCard = Boolean(pkg.darkCard);
                        const ctaClass = pkg.recommended
                            ...
                            : isDarkCard
                                ? "bg-white text-[#0070BB] hover:bg-zinc-100"
                                : "bg-zinc-900 text-white hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100";

                        return (
                            <div key={pkg.id} className={`relative flex flex-col rounded-3xl ring-1 transition-all ${cardClass}`}>
                                {pkg.recommended && (
                                ...
                                <p className={`mt-2 text-sm leading-tight ${pkg.recommended ? "opacity-95" : isDarkCard ? "opacity-95" : "opacity-80"}`}>
                                    {pkg.subtitle}
                                </p>
                                        ...
                                        <div className="mt-2 flex items-center justify-center gap-2">
                                            {pkg.slides.map((_, idx) => (
                                                <button
                                                    key={idx}
                                                    type="button"
                                                    onClick={() => setSlideIndexes((prev) => ({ ...prev, [pkg.id]: idx }))}
                                ...
                                <ul className={`flex-1 space-y-2 leading-tight ${pkg.recommended ? "text-xs opacity-100" : "text-sm opacity-90"}`}>
                                    {pkg.includes.map((feature) => (
                                        <li key={feature} className="flex items-start gap-2.5 leading-tight">
                                            <CheckCircle2
                                                size={18}
                                                className={pkg.recommended ? "text-yellow-400 dark:text-yellow-600" : isDarkCard ? "text-white/90" : "text-zinc-400"}
                                            />
                                            <span>{feature}</span>
                                        </li>
                                    ))}
                                ...
                                <div className={`mt-3 rounded-2xl p-3 ${pkg.recommended ? "text-xs" : "text-sm"} ${isDarkCard ? "bg-white/10" : ""}`}>
                                ...
                                {pkg.excludes && (
                                    <div
                                        className={`mt-3 rounded-2xl p-3 ${pkg.recommended ? "text-xs" : "text-sm"} ${isDarkCard ? "bg-white/15" : "bg-black/5 dark:bg-white/10"}`}
                                    >
                                ...
                                {pkg.ctaHref ? (
                                    <a
                                        ...
                                        className={`mt-8 w-full rounded-xl py-3 text-center text-sm font-semibold transition-transform active:scale-95 ${ctaClass}`}
                                    >
                                        {pkg.cta}
                                    </a>
                                ) : (
                                    <button className={`mt-8 w-full rounded-xl py-3 text-sm font-semibold transition-transform active:scale-95 ${ctaClass}`}>
                                        {pkg.cta}
                                    </button>
                                )}
                            </div>
